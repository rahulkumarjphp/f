 ...
 * @return boolean
 */
function smartic_is_product_archive() {
 ...
 */
function smartic_product_get_image($product){
 ...
 * @param $product WC_Product
 */
function smartic_product_get_price_html($product){
 ...
 *
 */
function smartic_get_previous_product($in_same_term = false, $excluded_terms = '', $taxonomy = 'product_cat') {
    $product = new Smartic_WooCommerce_Adjacent_Products($in_same_term, $excluded_terms, $taxonomy, true);
 ...
 *
 */
function smartic_get_next_product($in_same_term = false, $excluded_terms = '', $taxonomy = 'product_cat') {
    $product = new Smartic_WooCommerce_Adjacent_Products($in_same_term, $excluded_terms, $taxonomy);
    return $product->get_product();
}


function smartic_is_woocommerce_extension_activated($extension = 'WC_Bookings') {
...
function smartic_woocommerce_pagination_args($args) {
    $args['prev_text'] = '<i class="smartic-icon smartic-icon-chevron-left"></i>';
    $args['next_text'] ='<i class="smartic-icon smartic-icon-chevron-right"></i>';
    return $args;
}

add_filter('woocommerce_pagination_args', 'smartic_woocommerce_pagination_args', 10, 1);
...
if ( ! function_exists( 'wvs_taxonomy_meta_fields' ) ){
    function wvs_taxonomy_meta_fields( $field_id = false ) {
        ...
        $fields['color'] = array(
            array(
                'label' => esc_html__( 'Color', 'smartic' ), // <label>
                'desc'  => esc_html__( 'Choose a color', 'smartic' ), // description
        ...
        $fields['image'] = array(
            array(
                'label' => esc_html__( 'Image', 'smartic' ), // <label>
                'desc'  => esc_html__( 'Choose an Image', 'smartic' ), // description
