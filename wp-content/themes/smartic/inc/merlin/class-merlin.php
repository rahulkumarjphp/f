...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
	...
	protected function get_import_data( $selected_import_index = 0 ) {
        ...
        if ( ! empty( $import_files['options'] ) ) {
            $content['options'] = array(
                'title'            => esc_html__( 'Home Page', 'smartic' ),
                'description'      => esc_html__( 'Install homepage xml.', 'smartic' ),
                'pending'          => esc_html__( 'Pending', 'smartic' ),
                'installing'       => esc_html__( 'Installing', 'smartic' ),
                'success'          => esc_html__( 'Success', 'smartic' ),
                'install_callback' => array( $this->importer, 'import' ),
	...
	public function after_content_import_setup() {
		// Set static homepage.
		$homepage = smartic_get_page_by_title( apply_filters( 'merlin_content_home_page_title', 'Home' ) );
		...
		$blogpage = smartic_get_page_by_title( apply_filters( 'merlin_content_blog_page_title', 'Blog' ) );
	...
	public function before_content_import_setup() {
		...
		$hello_world = smartic_get_page_by_title( 'Hello World!', OBJECT, 'post' );
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		if ( isset($selected_import_data['import_rev_slider_file_url']) && ! empty( $selected_import_data['import_rev_slider_file_url'] ) ) {
            ...
            if (isset($selected_import_data['import_more_revslider_file_url']) && !empty($selected_import_data['import_more_revslider_file_url'])) {
