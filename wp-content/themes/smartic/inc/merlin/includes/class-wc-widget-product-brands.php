 ...
 * @extends WC_Widget
 */
class Smartic_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'smartic');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'smartic');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'smartic'),
				'label' => esc_html__('Title', 'smartic'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'smartic'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'smartic'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'smartic'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'smartic'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'smartic'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'smartic'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'smartic'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Smartic_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'smartic');
			...
			echo '<ul class="product-brands" id="smartic-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Smartic_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'smartic'),
		'singular_name'              => esc_html__('Brands', 'smartic'),
		'menu_name'                  => esc_html__('Brands', 'smartic'),
		'all_items'                  => esc_html__('All Brands', 'smartic'),
		'parent_item'                => esc_html__('Parent Brand', 'smartic'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'smartic'),
		'new_item_name'              => esc_html__('New Brand Name', 'smartic'),
		'add_new_item'               => esc_html__('Add New Brands', 'smartic'),
		'edit_item'                  => esc_html__('Edit Brand', 'smartic'),
		'update_item'                => esc_html__('Update Brand', 'smartic'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'smartic'),
		'search_items'               => esc_html__('Search Brands', 'smartic'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'smartic'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'smartic'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
